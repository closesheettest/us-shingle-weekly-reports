...
with tabs[0]:
    ...
    if uploaded_data is not None:
        try:
            ...
            # ---- Prep tagged dataframe for drilldown ----
            ...
            # ---- Read back summary tables from Excel ----
            ...
            with t1:
                ...
                for rep in rep_summary.iloc[:,0].astype(str).tolist():
                    if st.button(f"🔍 View {rep}", key=f"rep_{rep}"):
                        with st.modal(f"Sales Rep Details — {rep}"):
                            c1, c2, c3 = st.columns(3)
                            f_sit  = c1.checkbox("Only Sits", key=f"sits_{rep}")
                            f_sale = c2.checkbox("Only Sales", key=f"sales_{rep}")
                            f_ns   = c3.checkbox("Only No-Shows", key=f"noshow_{rep}")
                            mask = (df_tagged[cols_map["sales_rep"]].astype(str) == rep)
                            if f_sit: mask &= df_tagged["is_sit_sales"]
                            if f_sale: mask &= df_tagged["is_sale"]
                            if f_ns: mask &= df_tagged["is_no_show"]
                            st.dataframe(df_tagged.loc[mask, show_cols], use_container_width=True)
            ...
            with t3:
                ...
                for harv in harvester.iloc[:,0].astype(str).tolist():
                    if st.button(f"🔍 View {harv}", key=f"harv_{harv}"):
                        with st.modal(f"Harvester Details — {harv}"):
                            d1, d2 = st.columns(2)
                            hf_sit  = d1.checkbox("Only Sits", key=f"hsits_{harv}")
                            hf_sale = d2.checkbox("Only Sales", key=f"hsales_{harv}")
                            mask = (df_tagged["Harvester"].astype(str) == harv)
