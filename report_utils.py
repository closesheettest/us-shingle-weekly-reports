...
def load_config_from_path(config_path: Path):
    ...
    # Option: treat "No Sit - rescheduled" as a Sit for Harvester metrics (does NOT affect Sales)
    cfg["harvester_rescheduled_counts_as_sit"] = bool(
        cfg.get("harvester_rescheduled_counts_as_sit", False)
    )
    return cfg
...
def tag_statuses(df: pd.DataFrame, status_col: str, cfg) -> pd.DataFrame:
    ...
    Create flags:
      ...
      - is_sit_harvester: sales sit logic + treat 'New Roof' as a Sit
                          + (optional) treat 'No Sit - rescheduled' as a Sit if cfg flag is True
    ...
    # Harvester rules
    include_rescheduled = bool(cfg.get("harvester_rescheduled_counts_as_sit", False))
    resched_match = status_lc.eq("no sit - rescheduled")
    new_roof_match = status_lc.eq("new roof")

    df["is_sit_harvester"] = (
        df["is_sit_sales"] |
        new_roof_match |
        (include_rescheduled & resched_match)
    )
    return df
...
def compute_harvester_report(df: pd.DataFrame, cols) -> pd.DataFrame:
    ...
    harv = pd.DataFrame({
        ...
        "Sits": grp["is_sit_harvester"].sum(),  # harvester sit logic (may include rescheduled if enabled)
    })
